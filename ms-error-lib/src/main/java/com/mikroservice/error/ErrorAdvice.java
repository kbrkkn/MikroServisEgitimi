...
public class ErrorAdvice {
	...
	public ErrorObj handleException(final HttpStatusCodeException exp) {
		try {
			ErrorObj rootLoc = this.createBaseErrorObject().setErrorCode(3007)
					.setMessage("Error while calling another microservice");
			byte[] responseBodyAsByteArrayLoc = exp.getResponseBodyAsByteArray();
			ObjectMapper mapperLoc = new ObjectMapper();
			rootLoc.addSubError(mapperLoc.readValue(responseBodyAsByteArrayLoc, ErrorObj.class));
			return rootLoc;
		} catch (Exception eLoc) {
			return this.createBaseErrorObject().setErrorCode(5002).setMessage("Internal : " + exp.getMessage());
